...
# ==========================================
# SupCon training step
# ==========================================

def train_supcon_epoch(model, loader, optimizer, device,
                       temperature=None, log_every=50):
    """Train one epoch of MultiSupCon pretraining."""
    temperature = temperature or CFG["supcon_temperature"]
    model.train()
    running_loss = 0.0
    num_batches = 0
    ...
    for x1, x2, labels_ in progress_bar:
        x1, x2, labels_ = x1.to(device), x2.to(device), labels_.to(device)
...
# ==========================================
# Linear-probe AUROC harness (reused from simclr_encoder)
# ==========================================
...
def run_supcon_training(
    ...
    temperature=None,
    warmup_epochs=None,
    checkpoint_dir="checkpoints_supcon",
    device=DEVICE,
    seed=42,
):
    """Run MultiSupCon pretraining with warmup+cosine LR schedule, linear-probe
    AUROC evaluation, and early stopping."""

    # Resolve defaults from CFG
    epochs = epochs or CFG["supcon_epochs"]
    batch_size = batch_size or CFG["supcon_batch_size"]
    lr = lr or CFG["supcon_lr"]
    weight_decay = weight_decay or CFG["supcon_weight_decay"]
    temperature = temperature or CFG["supcon_temperature"]
    warmup_epochs = warmup_epochs or CFG["supcon_warmup_epochs"]
    ...
    supcon_dataset = ECGSSLWithLabels(x_train, y_train_supcon.to_numpy())
    ...
    for epoch in range(start_epoch, epochs):
        train_loss = train_supcon_epoch(
            model, supcon_loader, optimizer, device, temperature=temperature
        )
        scheduler.step()
        ...
        if (epoch + 1) % eval_every == 0 or (epoch + 1) == epochs:
            ...
            if auroc > best_auroc:
                ...
                print(f"      New best encoder saved (AUROC={best_auroc:.4f})")
            else:
                patience_counter += 1
    ...
    torch.save(model.encoder.state_dict(), "ecg_encoder_multisupcon_final.pth")
